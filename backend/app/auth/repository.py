...
async def get_account_by_email_ci(db: AsyncSession, email: str) -> Account | None:
    """Case-insensitive lookup by email. The real login-time lookup used by
    `auth/service.py::authenticate()` (replacing the old `_MOCK_ACCOUNTS`
    dict lookup). Also used by employees/service.py's IntegrityError
    backstop (Story 7.2 code review) to distinguish a conflict on the
    `accounts` side from one on `employees` after a rollback -- the two
    tables' email values aren't guaranteed to always match once a future
    story allows editing an Employee's email without also updating its
    paired Account (FR-26's own `[ASSUMPTION]` flags this exact drift as
    possible)."""
