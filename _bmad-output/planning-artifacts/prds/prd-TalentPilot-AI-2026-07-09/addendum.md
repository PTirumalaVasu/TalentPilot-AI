...
## Admin-Assisted Content Sourcing — Architecture Handoff Notes (2026-09-08, all items RESOLVED same day)

`prd.md` §4.6 (FR-16–FR-18) adds a live, per-Admin content-link lookup. All open questions — including Udemy's mid-day scope changes (dropped, then reopened once SAILS confirmed its existing Udemy for Business subscription, then fully resolved once the org-wide-credential shape was worked through) — came back resolved the same day. See `_bmad-output/planning-artifacts/architecture/architecture-TalentPilot-AI-2026-07-09/ARCHITECTURE-SPINE.md` (AD-7 amended + extended with a 3rd branch, AD-10 new + extended with `org_api_credentials`) for the binding decisions. This section is kept as the historical handoff record, with resolutions noted inline.

- **Data model implication — RESOLVED.** `content_catalog` gains `attached_by` (nullable admin-identity FK) and `origin` (`BATCH` default | `ADMIN_LOOKUP`) for FR-18's audit consequence. No `UDEMY` source value — Udemy dropped from v1 (below).
- **New credential storage — RESOLVED.** New `admin_api_keys` table (admin identity, source, encrypted key material, timestamps; unique per admin+source), owned by `content/` module. Confirmed distinct from — never reusing — the single shared `YOUTUBE_API_KEY` env var the batch job uses.
- **AD-7/AR-7 conflict — RESOLVED.** Amended (not replaced): a live per-request call is now permitted, but only from an authenticated HR Admin, using that Admin's own stored key (never the shared system key), writing to the catalog only on explicit review/attach. The batch-only rule stands unchanged for the shared-key path. The regression test's intent was re-scoped accordingly (guards the shared key's usage, not the search function itself).
- **Udemy API — researched, IN SCOPE via existing subscription (updated same day).** Udemy's free self-serve Affiliate API (get a key, search the public catalog — the YouTube-equivalent model this feature originally assumed) was discontinued 2025-01-01. The remaining Udemy APIs (Search, GraphQL, REST Courses) require an active **Udemy for Business** subscription and are scoped to that org's own licensed catalog, not the open marketplace. SAILS already holds this subscription, so Udemy is back in v1 scope (Open Question 14). Sources: `udemy.com/developers/affiliate`, `partnersupport.udemy.com` "Summary of Available APIs", `business-support.udemy.com` "Udemy Business Web APIs".
- **Udemy credential model — researched, RESOLVED.** Udemy for Business client ID/secret is issued **per organization** (one credential pair for the entire UFB account) — confirmed via `business-support.udemy.com` "How to Find Your Rest API Credentials in Udemy Business". This is architecturally distinct from YouTube's free, self-serve, per-developer personal key. **Resolution:** AD-7 extended with a third branch — a live, HR_ADMIN-gated Udemy lookup using the org-wide credential, with rate-limit failures surfaced via Udemy's own signal rather than a local counter (same principle already established for YouTube's batch quota, Story 2.3). AD-10 extended with a second table, `org_api_credentials` (attributed to whoever last configured it, not personally owned), kept separate from `admin_api_keys` so one table's uniqueness constraint doesn't have to express two different ownership models. A queue-to-batch alternative for Udemy specifically was considered and rejected — same reasoning as the original YouTube decision: it would make the two sources behave inconsistently within one "enter a skill, see links now" feature. Full detail: `ARCHITECTURE-SPINE.md` AD-7 (branch 3), AD-10.
- **Encryption mechanism — RESOLVED.** Fernet symmetric encryption (`cryptography` library) keyed by a new secret dedicated to this purpose, separate from the JWT signing secret. Zero new infrastructure — a KMS/envelope-encryption alternative was considered and rejected as disproportionate for this project's local-only, zero-budget scope (same reasoning that already ruled out a dedicated vector DB above).
