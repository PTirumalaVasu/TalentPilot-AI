...
16. **`[ADDED 2026-09-08]` Skills has never had a proper owning module — FR-20/21/22 make that a real gap, not a theoretical one.** The architecture spine (Story 2.3's scope notes) already flagged that `skills` isn't in AD-1's Binds list and no module owns it — until now, `skills` was read-only (batch ingestion enumerating names) and write-only via a bootstrap seed script, so the gap was low-stakes. FR-20/21/22 add real HR Admin-facing writes (create/edit/delete) with a business-critical invariant (permanently locked once ever-assigned) that needs a real enforcement point — a single owning module's service layer, per this codebase's established AD-1 pattern, not scattered checks. **Needs an architecture pass** (a new `skills/` module, or folding ownership into `assignments/` since it already seeds/reads Skills) before FR-20/21/22 are built — not assumed already covered by the existing spine.
17. **`[ADDED 2026-09-08]` An assigned Skill has no UI entry point into content-sourcing (FR-17/18/19/23) on the Skills tab, even though the FRs themselves state that capability is not gated by assignment status.** History: a "Find Content" button was added specifically to give assigned/locked Skills (which have no Edit control, FR-21/22's lock) a way to reach the lookup panel; removed the same day, from every card, per direct feedback, without a replacement named. **Two ways to resolve, not decided here:** (a) narrow FR-17/18/19/23's stated scope to unassigned Skills only, formally matching what the UI now actually allows — the simpler, more consistent option, but changes the FRs' own wording; or (b) design a different entry point for assigned Skills (e.g., reachable from the Provenance Drill-Down modal, 01.2, alongside other per-Assignment detail, rather than the Skills tab's card grid) that doesn't conflict with removing the card-level button. Revisit before FR-17/18/19/23 are built for the assigned-Skill case — not a launch-blocker for the unassigned-Skill path, which is unaffected.
