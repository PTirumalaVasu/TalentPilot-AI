...
- New architectural note: any future assignment-read call site (present or added later) must filter `active = true` — this is now a spine-level invariant for the `assignments` table, the same way `assignment_overrides` already has `idx_overrides_active ... WHERE active = true`.
