...
- [x] [Review][Fixed] ~~`onPlayerReady_Internal` attaches `visibilitychange`/`beforeunload` listeners before `cleanupListenersRef.current` is assigned...~~ **Correction (Round 2, Acceptance Auditor): this line was stale/inaccurate.** It claimed the ordering was "unchanged by this diff's edits... only the catch block's error handling was touched" — false; the diff explicitly moved the `cleanupListenersRef.current` assignment earlier, before `onPlayerReady?.()`. Actually fixed as a byproduct of the attempt-token restructuring (matches `deferred-work.md`'s parallel entry, which had it right all along). No longer deferred.
...
### Review Findings — Round 2 (2026-07-11, post-patch, on committed `391bf59`)

User-requested independent re-review of the just-patched, just-committed diff (3 fresh adversarial layers, instructed not to assume Round 1's fixes were correct). Found 1 critical regression introduced by Round 1's own `.destroy()` patch, 1 more real gap in the same area, 1 stale self-report in this story's own Round 1 checklist, and 3 minor hardening items. 0 decision-needed, 6 patches, 3 deferred, 4 dismissed. All 6 patches applied 2026-07-11.

- [x] [Review][Patch][CRITICAL] **Round 1's `playerRef.current.destroy()` fix (patch 2) introduced a real, empirically-reproduced regression under React StrictMode.** `main.tsx` wraps the app in `<React.StrictMode>`, which mount→unmount→remounts every component's effects once in dev. The unmount cleanup now calls `.destroy()` on `captureServiceRef`/`adapterRef`/`playerRef` but never nulls any of the three refs afterward. On the StrictMode remount, `initPlayer()`'s own guard (`if (!containerRef.current || playerRef.current) return;`) sees the stale non-null `playerRef.current` — pointing at the already-destroyed player — and silently no-ops. No new player/adapter/capture service is ever created; `isReady` (set `true` by the first, now-dead attempt) stays `true`, so the UI keeps showing "Capture service active" while watch-progress capture is permanently dead for the rest of the session. Reproduced directly by the reviewing subagent via a throwaway probe test mounting `<VideoPlayer>` inside `<React.StrictMode>`. Invisible to this diff's own 7 tests since none of them render under `StrictMode` or exercise unmount at all. [frontend/src/components/VideoPlayer.tsx: unmount cleanup, ~lines 86-104] — **fixed**: extracted a shared, exception-safe `destroyPlayerResources()` helper (used by both unmount cleanup and `handleRetry`) that destroys *and nulls* all four refs (`cleanupListenersRef`, `captureServiceRef`, `adapterRef`, `playerRef`). Verified red→green: confirmed the new StrictMode regression test fails (`PlayerCtor` called only once) with the nulling temporarily removed, and passes (`PlayerCtor` called twice) with the fix in place.
- [x] [Review][Patch] `handleRetry` has no `isMountedRef` check at its top, and neither `handleRetry`'s nor the unmount cleanup's teardown sequence is exception-safe — a throw from any single `.destroy()` call (e.g. the real `YT.Player.destroy()`) halts the remaining teardown steps, leaving `playerRef.current` non-null forever and permanently disabling all future retries (or, on unmount, leaking whatever didn't get destroyed). [frontend/src/components/VideoPlayer.tsx: `handleRetry`, unmount cleanup] — fixed as part of the same `destroyPlayerResources()` extraction: each of the 4 teardown steps is independently wrapped in `try`/`catch` (logged via `console.error`, never blocks the rest), and `handleRetry` now starts with `if (!isMountedRef.current) return;`.
- [x] [Review][Patch] This story's own Round 1 `[Review][Defer]` bullet for the listener-cleanup-ordering item is now factually stale — it says the ordering fix left that area "unchanged by this diff's edits," but the diff explicitly moved `cleanupListenersRef.current`'s assignment earlier. `deferred-work.md`'s parallel entry already correctly says it was fixed as a byproduct; this story file's own checklist line contradicts it and needs to match. — fixed: the Round 1 bullet above is corrected in place (marked `[Review][Fixed]` with a correction note) rather than left contradicting `deferred-work.md`.
- [x] [Review][Patch] The freshly-created `playerHost` div reuses the same DOM id (`youtube-player-${assignmentId}`) on every retry attempt — currently safe only by accident of synchronous JS execution ordering (the old node is removed before the new one with the same id is appended), but a landmine for any future code that queries by that id and holds a reference across a retry. [frontend/src/components/VideoPlayer.tsx: `initPlayer`] — fixed: id now includes `myAttempt` (`youtube-player-${assignmentId}-${myAttempt}`), unique per attempt regardless of execution order.
- [x] [Review][Patch] The "second failed retry" test doesn't assert the `PlayerCtor` call count increments to 3 (unlike the first retry test, which does assert a call-count increment) — weaker coverage than its name implies; wouldn't catch `handleRetry` degenerating into a UI-only toggle that stops actually re-attempting construction. [frontend/src/components/VideoPlayer.test.tsx] — fixed: test renamed and now asserts `PlayerCtor` called 2 times after the first retry and 3 times after the second.
- [x] [Review][Patch] The `containerRef` comment overclaims precedent — it says the fresh-container pattern "match[es]" `VideoPlayerDemo.tsx`'s established approach, but `VideoPlayerDemo.tsx` never calls `.destroy()` on the raw player object (only on its adapter/capture wrappers), so it establishes no tested precedent for this diff's new `playerRef.current.destroy()` call specifically. [frontend/src/components/VideoPlayer.tsx: `containerRef` comment] — fixed: comment narrowed to claim precedent for the DOM-recreation approach only, explicitly noting `VideoPlayerDemo.tsx` doesn't call `.destroy()` on the raw player.

Also added: a new regression test (`"correctly re-initializes after React.StrictMode's dev-mode mount->unmount->remount cycle"`) that renders `<VideoPlayer>` inside `<StrictMode>` and asserts the player is constructed twice — the direct, permanent proof that the critical fix above actually works, not just that the code reads correctly.

- [x] [Review][Defer] `onPlayerReady_Internal` has no idempotency guard against a hypothetical double `onReady` firing for the same (still-current) attempt — theoretical, no established real-world YouTube IFrame API behavior triggers this, and it's a pre-existing shape of the function not made worse by this diff. Revisit only if a real duplicate-`onReady` report surfaces.
- [x] [Review][Defer] The "Try again" button has no `aria-describedby`/`aria-label` linking it back to the alert message text — a real a11y nice-to-have (Blind Hunter), but beyond what AC3/AC6 literally require (icon+text, `role="alert"` on the message). Revisit alongside the already-deferred `Combobox`/loading-state `aria-live` gaps logged in `deferred-work.md`.
- [x] [Review][Defer] `onError` prop callback still receives the raw/technical diagnostic message (e.g. "Video not found..."), not the generic literal copy — re-raised by Round 2's Blind Hunter, same reasoning as Round 1's dismissal still applies (no AC requires sanitizing the programmatic callback, no current consumer exists) — kept as a defer rather than a repeat-dismiss since it's now been raised twice and is worth a permanent note rather than re-litigating each round.

**Dismissed as noise or already-handled (Round 2)**: an untracked `frontend/src/components/VideoPlayer.strictmode.test.tsx` file and an apparent "26 vs 25 `tsc` errors" discrepancy the Acceptance Auditor flagged — resolved: this was the Blind Hunter subagent's own throwaway StrictMode probe test, created and self-deleted during its investigation (confirmed by Blind Hunter's own report), caught mid-existence by the Acceptance Auditor running concurrently against the same live working directory. Confirmed after the fact: the file does not exist, `tsc --noEmit` reports exactly 25 errors. A review-process artifact from two subagents running concurrently, not a real code defect; unguarded new `console.error(...)` calls — working as intended per this story's own Dev Notes (raw diagnostics for debugging, never shown in the UI); pre-existing `VideoPlayer.tsx` `tsc` errors (`CaptureService`-as-type, `UUID` branding) — already logged/deferred three times now (Story 3.4, this story's own Task 0 finding, Round 1 review), not re-logged again; `error` state theoretically vulnerable to a future `setError('')` silently hiding a real error — no actual code path does this today, overly speculative.
...
- 2026-07-11: All 8 code-review Patch findings applied — per-attempt token/mounted guard (`attemptIdRef`/`isMountedRef`) against stale async callbacks and double-click retry races; `handleRetry` now calls the real `YT.Player.destroy()` before discarding it; the reused-stale-DOM-node risk fixed by switching to a stable `containerRef` with a fresh child element per attempt (matching `VideoPlayerDemo.tsx`'s established pattern); `role="alert"` scoped to the message text only, no longer wrapping the "Try again" button; 2 new tests added (`onError` prop callback fires on failure; `onPlayerReady_Internal`'s own try/catch reaches the same error+retry UI); `deferred-work.md`'s prose breakdown corrected (20 errors / 1 real `error TS6133`, not "19... plus one warning"); error copy deduplicated behind the exported `VIDEO_LOAD_FAILURE_MESSAGE` constant, imported by the test file instead of re-hardcoded. Verified: `npx vitest run` 155/155 passing, `npx tsc --noEmit` unchanged at 25 pre-existing errors, `npx vite build` clean. Status → `done`. (Committed as `391bf59`.)
- 2026-07-11: **Round 2 code review** (user-requested independent re-review of the committed `391bf59` diff, 3 fresh adversarial layers instructed not to assume Round 1's fixes were correct). Found that Round 1's own `playerRef.current.destroy()` patch introduced a real regression: the unmount cleanup destroyed but never nulled `playerRef`/`adapterRef`/`captureServiceRef`, so under React StrictMode's dev-mode mount→unmount→remount cycle (which `main.tsx` actually uses), the remount's `initPlayer()` guard saw a stale destroyed `playerRef` and silently no-op'd — the capture pipeline died permanently while the UI kept claiming "Capture service active." Empirically reproduced by the reviewing subagent via a throwaway probe test before this fix. All 6 patches applied: extracted a shared, exception-safe `destroyPlayerResources()` helper (destroys *and nulls* all 4 refs, each independently try/catch-wrapped) used by both unmount cleanup and `handleRetry`; added an `isMountedRef` check to `handleRetry`; corrected this story's own stale Round 1 `[Review][Defer]` bullet (Acceptance Auditor caught it contradicting `deferred-work.md`); made the per-attempt DOM host id unique (`-${myAttempt}` suffix); strengthened the "second failed retry" test to assert a 3rd construction attempt; narrowed an overclaiming code comment. Added a new permanent regression test proving the fix — confirmed red (fails, player constructed only once) with the ref-nulling temporarily removed, green (constructed twice) with the fix in place. Final: 156/156 frontend tests passing (up from 155), `tsc` unchanged at 25 pre-existing errors, `vite build` clean. Status remains `done`.
