...
- **Frontend full suite**: 217 passed / 2 failed out of 219 — both pre-existing, unrelated `DashboardPage.test.tsx` failures ("renders loading state on mount", "pagination shows correct page numbers and total"), unchanged in count since Story 5.4/5.5.
...
- Test counts: backend `test_override_endpoint.py` 17/17 (16 carried over from Story 5.5 + 1 new for this story's AC6); `test_drill_down_endpoint.py`/`test_provenance_detail.py` 19/19 unchanged (proves zero regression from the pure `UnderlyingSignal` extraction refactor). Frontend: `ProvenanceDrillDownModal.test.tsx` 28/28 (3 stale placeholder tests from Story 5.5 rewritten to match the real Reverse-Override behavior, 9 new reversal-flow tests added). Full frontend suite 217/219 (2 pre-existing unrelated failures, unchanged).
...
- `frontend/src/features/dashboard/ProvenanceDrillDownModal.test.tsx` — 3 stale Story-5.5-era placeholder tests (asserting the old disabled-stub behavior) rewritten to match the real Reverse-Override behavior; 9 new tests added under a "Reverse Override confirm flow (Story 5.5b)" describe block
