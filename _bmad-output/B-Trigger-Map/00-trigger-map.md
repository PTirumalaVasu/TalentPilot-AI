...
- **Scored, MVP already committed, `[ADDED 2026-09-13, RE-EVALUATED 2026-09-15]`:** Skill Assignment Dashboard (org-wide stats + Assignment Progress ring) — originally scored Medium/Primary (3) for its Employee Segmentation pie chart's connection to Rita's Want #3, but **that chart was removed 2026-09-15 (Story 10.11), replaced by the Experience Distribution panel** — the mechanic the score was earned by no longer exists on this page. Score left unrevised, flagged for a deliberate re-score next time this document is touched. See [06-Feature-Impact.md](06-Feature-Impact.md)'s re-evaluation note.
- **Enabling (outside the scoring model), `[ADDED 2026-09-15]`:** Epic 10 (Post-MVP Admin & Roster Refinements, FR-34–40) — roster grid columns/flag/panel extend the already-Enabling Employee Roster Management; Skills/Skill-Assignments search+pagination is pure findability tooling; the new-Skill content-sourcing fix hardens the already-Must-Have HR Assignment Flow rather than adding new psychological pull. See [06-Feature-Impact.md](06-Feature-Impact.md)'s Enabling Features section, which also flags a pre-existing gap found in this pass: Epic 6 (Admin-Assisted Content Sourcing) was never run through Trigger Mapping at all.
