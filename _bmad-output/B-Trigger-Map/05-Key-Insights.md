...
### Skill Assignment Dashboard (Landing Page) Must: `[ADDED 2026-09-13, REVISED 2026-09-15]`
- `[SUPERSEDED 2026-09-15]` ~~Answer "how's the org doing overall, and who needs my attention" as a genuine complement to Want #3... Treat the "Needs Attention" pie segment with the same non-negotiable legibility rules... clicking it must lead straight to the specific employees behind that count (FR-33)...~~ **No longer applicable — Story 10.11 removed the Employee Segmentation pie chart and its Needs Attention click-through from this page entirely, replaced by the Experience Distribution panel (FR-36).** This page no longer answers "who needs my attention" in any form — that question is answered only inside the full grid now (FR-9/FR-10, via `Skill Assignments`). Kept struck through, not deleted, so a future reader can see what changed and why (see `06-Feature-Impact.md`'s 2026-09-15 re-evaluation note).
- `[RESOLVED 2026-09-15, differently than expected]` The `[FLAGGED]` note that used to sit here — this page being "the closest thing that has actually shipped to the Needs Attention filter" scored in Phase 2 — is now moot: the thing that made it close (the segmentation chart) is gone. The original Phase 2 "Needs Attention Filter" gap (`06-Feature-Impact.md`'s flagged drift) is **fully unresolved again**, not partially resolved — worth knowing if anyone revisits that old scoring line.
- The Experience Distribution panel that replaces it (FR-36) must follow the same non-color-only legibility rule already locked for every chart in this document — but it is explicitly **not** a readiness signal and must not be styled or worded in a way that implies it is (e.g., no red/amber/green tinting suggesting urgency) — it's a headcount breakdown, a different kind of information entirely from what used to occupy this space.
- Stay a genuinely optional first glance, not a gate Rita has to pass through — UJ-1's validated grid-and-drill-down flow (unchanged, §4.10/FR-33 path (a)) must keep working exactly as it does today for a Rita who skips the landing page's stats entirely and goes straight to `Skill Assignments`. This constraint survives the Story 10.11 change unchanged.

### Post-MVP Admin & Roster Refinements Must: `[ADDED 2026-09-15]`
- Not reintroduce Fear #3 ("the chore just relocates") — same standing test already applied to Employee Roster Management above, now extended to its FR-34–36 refinements (more grid columns, a Talent-Pool flag, an Experience Distribution panel). Adding fields/panels to a roster screen is easy to justify one at a time; the cumulative effect on Rita's actual time-in-screen is what matters, not any single addition in isolation.
- Keep the 90-day Talent-Pool flag (FR-35) visually and linguistically distinct from the dashboard's Needs-Attention staleness flag (FR-10) — both are red, both say "stale," but they mean different things (an Employee's tenure vs. an Assignment's readiness signal). Conflating them in the UI would reintroduce exactly the kind of ambiguous-status-word failure mode named in Critical Success Factors above, just on a new screen.
- Search/pagination additions to the Skills tab and Skill Assignments grid (FR-37, FR-38) must not change any existing row's Status/Provenance behavior — these are pure findability additions layered on top of the already-locked "provenance always paired with text, one-click drill-down on every row" rules above, not a redesign of them.
- The new-Skill content-sourcing fix (FR-39) exists specifically because a real HR Admin hit "I created a Skill and couldn't find any video to assign" — this directly threatens the HR Assignment Flow's own Design Implication above ("collapsing 'assign' and 'here's what to watch' into one action"). Treat this as closing a real crack in an already Must-Have mechanic, not a nice-to-have polish item.
